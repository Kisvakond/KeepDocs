...
if (_canDo) then {
	...
	if (true == DZE_KeepHowTos) then {
		...
		for "_x" from 0 to ((count _selectedRecipeInput) - 1) do {
			...
			_itemIn = format["%1", ((_selectedRecipeInput select _x) select 0)];
			if (    (_itemIn == format ["%1", _item]) && {(_itemIn != "ItemDocument")} ) exitWith {
				/* if the input item is the HowTo itself but not an unexamined document,
				   let's keep the HowTo by removing it from the list of input stuff.*/
				/* Workaround: nested arrays can't be subtracted. Replace the located element with a single value */
				_selectedRecipeInput set [_x, "CLEARME"];
				_selectedRecipeInput = _selectedRecipeInput - ["CLEARME"];
