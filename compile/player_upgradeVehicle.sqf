		...
		if (!isNil "_upgrade" && (count _upgrade) > 0) then {
			...
			if (true == DZE_KeepHowTos) then {
				...
				for "_x" from 0 to ((count _requirementsMagazine) - 1) do {
					...
					if ( (format["%1", ((_requirementsMagazine select _x) select 0)]) == (format ["%1", _this])) exitWith {
						/* if the input item is the HowTo itself, let's keep the HowTo by removing it from the list of input stuff.*/					
						/* Workaround: nested arrays can't be subtracted. Replace the located element with a single value */					
						_requirementsMagazine set [_x, "CLEARME"];
						_requirementsMagazine = _requirementsMagazine - ["CLEARME"];
					};
				};
			};
...

				{
...

					{
						if( (_removed < _countIn) && ((_x == _itemIn) || configName(inheritsFrom(configFile >> "cfgMagazines" >> _x)) == _itemIn)) then {							
							/* Don't consume the howto used for this upgrade */
							/*if ( _upgradeName == format ["%1", _x] ) then {
								_num_removed = 1;
							}
							else {*/
